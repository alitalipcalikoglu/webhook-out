...
export class SubscriptionStore {
  static COLUMNS = 'id, name, description, url, events, headers, secret_enc, prev_secret_enc, prev_until, status, consecutive_failures, last_delivery_at, last_status, created_by, created_at, updated_at';
  ...
  constructor(db) {
    ...
    this.stmt = {
      ...
      update: db.prepare(`UPDATE subscriptions SET name = ?, description = ?, url = ?, events = ?, headers = ?, secret_enc = ?, prev_secret_enc = ?, prev_until = ?, status = ?, consecutive_failures = ?, updated_at = ? WHERE id = ?`),
  ...
  insert(r) {
    this.stmt.insert.run(r.id, r.name, r.description, r.url, r.events, r.headers, r.secret_enc, r.prev_secret_enc, r.prev_until, r.status, r.consecutive_failures, r.last_delivery_at, r.last_status, r.created_by, r.created_at, r.updated_at);
    return r;
  }
  ...
  /** @param {SubscriptionRow} r */
  update(r) {
    this.stmt.update.run(r.name, r.description, r.url, r.events, r.headers, r.secret_enc, r.prev_secret_enc, r.prev_until, r.status, r.consecutive_failures, r.updated_at, r.id);
    return r;
  }
