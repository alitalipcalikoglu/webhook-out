...
test('DeliveryStore: claim hands out a fresh owner_token and lease_until per row', () => {
  ...
  const [d1, d2] = t.deliveries.claim(t.clock.now(), 2, 30_000, 1_000_000);
...
test('DeliveryStore: finish is a no-op once the owner_token no longer matches (fencing)', () => {
  const t = testService();
  const d = seedOne(t);
  const [claimed] = t.deliveries.claim(t.clock.now(), 1, 30_000, 1_000_000);
...
test('DeliveryStore: reclaimExpired is atomic against a concurrent heartbeat for the same row', () => {
  const t = testService();
  const d = seedOne(t);
  t.deliveries.claim(t.clock.now(), 1, 10_000, 1_000_000);
...
test('DeliveryStore: reclaimExpired ignores a row whose lease was renewed before the sweep', () => {
  const t = testService();
  const d = seedOne(t);
  const [claimed] = t.deliveries.claim(t.clock.now(), 1, 10_000, 1_000_000);
  const token = /** @type {string} */ (claimed.owner_token);
...
test('Worker: recover() only reclaims EXPIRED leases, not a lease still within its TTL', () => {
  ...
  const d = seedOne(t);
  t.deliveries.claim(t.clock.now(), 1, 5_000, 1_000_000);
  t.worker.recover();
  assert.equal(t.deliveries.get(d.id)?.status, 'running');
...
test('Worker: heartbeat keeps a long in-flight call owned across the original lease window', async (t) => {
  ...
  const worker = new Worker({ events: eventService, subscriptionService, subscriptions, deliveries, eventStore: events, presence, caller, log: silent, options: { concurrency: 1, pollMs: 50, retentionDays: 30, disableAfterFailures: 10, leaseMs: 120, heartbeatMs: 40, drainMs: 5_000, subscriptionConcurrencyMax: 1_000_000 } });
...
test('DeliveryStore: reclaimExpired exact-boundary invariant — now == lease_until is NOT yet expired (Stage 6.1)', () => {
  const t = testService();
  const d = seedOne(t);
  const [claimed] = t.deliveries.claim(t.clock.now(), 1, 1_000, 1_000_000);
